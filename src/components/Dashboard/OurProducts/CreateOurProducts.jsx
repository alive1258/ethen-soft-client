...
import { useCreateOurProductsMutation } from "@/redux/api/ourProductApi";

const CreateOurProducts = () => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const [createOurProduct, { isLoading }] = useCreateOurProductsMutation();
  ...
  const onSubmit = async (data) => {
    try {
      const res = await createOurProduct({
        ...data,
        slug,
        description: content,
      }).unwrap();
      if (res?.success === true) {
        reset();
        router.back();
        toast.success("Our Products added successfully!", {
    ...
    <div className="max-w-[1000px] bg-black-muted text-[#ADB5BD] mx-auto my-10 p-8 rounded-lg">
      ...
      >
        <div className="flex flex-col gap-2">
          ...
          {/* color_code */}
          <div className="mt-2">
            <span className="text-[16px] py-2"> color_code *</span>
            <input
              className="w-full rounded-lg border-[1.5px] border-stroke bg-transparent py-3 px-5 text-black outline-none transition focus:border-info-base active:border-primary-base disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input text-black dark:focus:border-primary"
              type="text"
              placeholder="Please enter your works type"
              {...register("color_code", {
                required: "Our Works color_code is required",
              })}
            />
            {errors.color_code && (
              <span className="text-red-600 text-sm">
...
export default CreateOurProducts;
