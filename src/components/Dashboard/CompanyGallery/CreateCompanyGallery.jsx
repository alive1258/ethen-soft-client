...
import { useCreateCompanyGalleryMutation } from "@/redux/api/companyGalleriesApi";
import { useRouter } from "next/navigation";
import { useForm } from "react-hook-form";
import { toast } from "react-toastify";

const CreateCompanyGallery = () => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const [createCompanyGallery, { isLoading }] =
  ...
  const onSubmit = async (data) => {
    try {
      const res = await createCompanyGallery({ ...data }).unwrap();
      if (res?.success === true) {
        reset();
        router.back();
        toast.success("CompanyGallery added successfully!", {
...
export default CreateCompanyGallery;
