...
const UpdateBanners = ({ id }) => {
  ...
  const { data, isLoading: fetchLoading, error } = useGetSingleBannerQuery(id);

  const [updateBanners, { isLoading }] = useUpdateBannerMutation();
  ...
  return (
    <div className="max-w-[1000px] bg-black-muted text-[#ADB5BD] mx-auto my-10 p-8 rounded-lg">
      <h1 className="text-[#ADB5BD] text-[23px] font-bold">Update Banner</h1>
      <form
        onSubmit={handleSubmit(onSubmit)}
        className="flex flex-col space-y-3 mt-4"
      >
        <div className="flex flex-col gap-2">
          {/* Input for Bannersname */}
