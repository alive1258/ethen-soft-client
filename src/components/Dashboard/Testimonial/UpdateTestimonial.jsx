...
const UpdateTestimonials = ({ id }) => {
  ...
  const {
    ...
    error,
  } = useGetSingleTestimonialsQuery(id);

  const [updateTestimonials, { isLoading }] = useUpdateTestimonialsMutation();
  ...
  useEffect(() => {
    if (data) {
      ...
      setValue("profession", data?.data?.profession || ""); // Set initial image value
      setValue("description", data?.data?.description || ""); // Set initial image value
      setValue("rating", data?.data?.rating || ""); // Set initial image value
    }
  }, [data, setValue]);
    ...
    <div className="max-w-[1000px] bg-black-muted text-[#ADB5BD] mx-auto my-10 p-8 rounded-lg">
      ...
      >
        <div className="flex flex-col gap-2">
          {/* Input for Testimonialsname */}
          <div className="relative w-full">
            <span className="text-[16px] py-2 block">Testimonials name *</span>
            <input
              ...
              type="text"
              placeholder="Please enter your Testimonials name"
              {...register("name", {
