...
import { useCreateOurClientsMutation } from "@/redux/api/ourClientsApi";
...
const CreateClients = () => {
  ...
  const [createClients, { isLoading }] = useCreateOurClientsMutation();

  const router = useRouter();

  const onSubmit = async (data) => {
    try {
      const res = await createClients({ ...data }).unwrap();
    ...
    <div className="max-w-[1000px] bg-black-muted text-[#ADB5BD] mx-auto my-10 p-8 rounded-lg">
      ...
      >
        <div className="flex flex-col gap-2">
          {/* Input for Clients name */}
          <div className="relative w-full">
            <span className="text-[16px] py-2 block">Clients name *</span>
            <input
              ...
              type="text"
              placeholder="Please enter your Clients name"
              {...register("name", {
                required: "Clients name is required",
              })}
            />
            {errors.name && (
