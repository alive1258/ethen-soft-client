...
const UpdateProfessionalService = ({ id }) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const {
    ...
    error,
  } = useGetSingleProfessionalServiceQuery(id);

  const [updateProfessionalServices, { isLoading }] =
    useUpdateProfessionalServicesMutation();
  ...
  useEffect(() => {
    if (data) {
      ...
      setValue("service_one", data?.data?.service_one || ""); // Set initial image value
      setValue("service_tow", data?.data?.service_tow || ""); // Set initial image value
      setValue("service_three", data?.data?.service_three || ""); // Set initial image value
      setValue("service_four", data?.data?.service_four || ""); // Set initial image value
  ...
  const onSubmit = async (data) => {
    try {
      const res = await updateProfessionalServices({
        id,
        data,
      }).unwrap();
      ...
      if (res?.success === true) {
        router.back();
        toast.success("ProfessionalServices updated successfully!", {
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="relative w-full">
            <span className="text-[16px] py-2 block">
              Our ProfessionalServices title *
            </span>
            <input
              ...
              type="text"
              placeholder="Please enter your ProfessionalServices title"
              {...register("title", {
                required: "ProfessionalServices title is required",
              })}
            />
            {errors.title && (
