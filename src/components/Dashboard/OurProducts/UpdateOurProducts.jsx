...
const UpdateOurProducts = ({ id }) => {
  ...
  const {
    ...
    error,
  } = useGetSingleOurProductQuery(id);

  const [updateOurProduct, { isLoading }] = useUpdateOurProductsMutation();
  ...
  const ourProductId = data?.data?._id;
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const res = await updateOurProduct({
        id: ourProductId,
        data,
      }).unwrap();
...
export default UpdateOurProducts;
