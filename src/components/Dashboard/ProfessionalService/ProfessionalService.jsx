...
} from "@/redux/api/professionalServicesApi";

const ProfessionalService = () => {
  const { data, error, isLoading } = useGetAllProfessionalServicesQuery();

  const [deleteProfessionalService] = useDeleteProfessionalServiceMutation();

  const handleDeleteProfessionalServices = async (data) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the  ProfessionalServices "${data?.title}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteProfessionalService(data?._id).unwrap();
        if (response?.success === true) {
          Swal.fire({
            title: "Deleted!",
            text: `The ProfessionalServices "${data?.title}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
      ...
      console.error("Delete ProfessionalServices error:", error);
      Swal.fire({
        title: "Error!",
  ...
  return (
    <>
      <div className="h-[75px] md:px-10 mt-5 rounded-t-[26px] flex justify-between items-center px-3 text-[#ADB5BD] bg-black-muted">
        ...
        <Link
          href={"/dashboard/admin/about/professional-services/create"}
              ...
              <tbody>
                {data?.data?.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-4">{item?.service_tow}</td>
                    <td className="py-3 px-4">
                    ...
                    <td className="my-2 px-4 text-end rounded-r-xl">
                      <div className="flex items-center justify-end w-full gap-4">
                        <Link
                          href={`/dashboard/admin/about/professional-services/update/${item?._id}`}
                        >
                          <LiaEditSolid className="text-info-base text-2xl" />
                        </Link>
                        <button
                          onClick={() => handleDeleteProfessionalServices(item)}
                        >
