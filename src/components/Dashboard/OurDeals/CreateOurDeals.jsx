...
import { useCreateOurDealsMutation } from "@/redux/api/ourDealsApi";
import { useRouter } from "next/navigation";
import { useForm } from "react-hook-form";
import { toast } from "react-toastify";

const CreateOurDeals = () => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const [createOurDeals, { isLoading }] = useCreateOurDealsMutation();

  const router = useRouter();

  const onSubmit = async (data) => {
    try {
      const res = await createOurDeals({ ...data }).unwrap();

      if (res?.success === true) {
        reset();
        router.back();
        toast.success("OurDeals added successfully!", {
