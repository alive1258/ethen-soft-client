...
const PrivacyPolicy = () => {
  ...
  const handleDeletePrivacyPolicy = async (data) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the  PrivacyPolicy "${data?.name}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deletePrivacyPolicy(data?._id).unwrap();
        if (response?.success === true) {
          Swal.fire({
            title: "Deleted!",
            text: `The PrivacyPolicy "${data?.title}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
    ...
    <>
      <div className="h-[75px] md:px-10 mt-5 rounded-t-[26px] flex justify-between items-center px-3 text-[#ADB5BD] bg-black-muted">
        <div className="flex items-center gap-1 text-[#ADB5BD] text-base font-semibold">
          <PiAlarmFill />
          <span>All PrivacyPolicy</span>
        </div>
        <Link
          href={"/dashboard/admin/setting/privacy-policies/create"}
          className="flex items-center gap-1 cursor-pointer text-[#4D69FA] bg-[#F0EFFB] rounded-[50px] py-1 px-[10px]"
        >
          <MdOutlineCloudUpload />
          <span className="text-[13px] font-semibold">Add PrivacyPolicy</span>
        </Link>
      </div>
                  ...
                  >
                    ...
                    <td className="my-2 px-4 text-end rounded-r-xl">
                      <div className="flex items-center justify-end w-full gap-4">
                        <Link
                          href={`/dashboard/admin/setting/privacy-policies/update/${item?._id}`}
                        >
                          <LiaEditSolid className="text-info-base text-2xl" />
                        </Link>
                        <button onClick={() => handleDeletePrivacyPolicy(item)}>
