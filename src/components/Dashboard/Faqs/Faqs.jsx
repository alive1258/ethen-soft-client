...
import { useDeleteFaqsMutation, useGetAllFaqsQuery } from "@/redux/api/faqsApi";
...
const Faqs = () => {
  ...
  if (error) {
    console.error("Error fetching Faqs descriptions:", error);
    return <p>Error fetching Faqs descriptions.</p>;
  }

  const handleDeleteFaqs = async (data) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the Faqs "${data?.title}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteFaqs(data?._id).unwrap();
        if (response?.success === true) {
          Swal.fire({
            title: "Deleted!",
            text: `The Faqs "${data?.title}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
                  ...
                  >
                    ...
                    <td className="my-2 px-4 text-end rounded-r-xl">
                      <div className="flex items-center justify-end w-full gap-4">
                        <Link
                          href={`/dashboard/admin/home/faqs/update/${item?._id}`}
                        >
                          <LiaEditSolid className="text-info-base text-2xl" />
                        </Link>
                        <button onClick={() => handleDeleteFaqs(item)}>
