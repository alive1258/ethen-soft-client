...
const OurWorks = () => {
  const { data, error, isLoading } = useGetAllOurWorksQuery();

  const [deleteOurWorks] = useDeleteOurWorkMutation();

  const handleDeleteOurWorks = async (data) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the Our Works "${data?.title}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteOurWorks(data?._id).unwrap();
        if (response?.success === true) {
          Swal.fire({
            title: "Deleted!",
            text: `The Our Works "${data?.title}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
      ...
      console.error("Delete Our Works error:", error);
      Swal.fire({
        title: "Error!",
    ...
    <>
      <div className="h-[75px] md:px-10 mt-5 rounded-t-[26px] flex justify-between items-center px-3 text-[#ADB5BD] bg-black-muted">
        <div className="flex items-center gap-1 text-[#ADB5BD] text-base font-semibold">
          <PiAlarmFill />
          <span>All Our Works</span>
        </div>
        <Link
          href={"/dashboard/admin/home/our-works/create"}
          className="flex items-center gap-1 cursor-pointer text-[#4D69FA] bg-[#F0EFFB] rounded-[50px] py-1 px-[10px]"
        >
          <MdOutlineCloudUpload />
          <span className="text-[13px] font-semibold">Add Our Works</span>
        </Link>
      </div>
              ...
              <tbody>
                {data?.data?.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-4">{item?.tech}</td>
                    <td className="my-2 px-4 text-end rounded-r-xl">
                      <div className="flex items-center justify-end w-full gap-4">
                        <Link
                          href={`/dashboard/admin/home/our-works/update/${item?.slug}`}
                        >
                          <LiaEditSolid className="text-info-base text-2xl" />
                        </Link>
                        <button onClick={() => handleDeleteOurWorks(item)}>
