  ...
  const handleDeleteBlogs = async (data) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the  Blogs "${data?.title}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        ...
        if (response?.success === true) {
          Swal.fire({
            title: "Deleted!",
            text: `The Blogs "${data?.title}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
