...
const Banner = () => {
  ...
  const [deleteBanners] = useDeleteBannerMutation();

  const handleDeleteBanners = async (data) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the  Banners "${data?.name}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteBanners(data?._id).unwrap();
        if (response?.success === true) {
          Swal.fire({
            title: "Deleted!",
            text: `The Banners "${data?.title}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
                  ...
                  >
                    ...
                    <td className="my-2 px-4 text-end rounded-r-xl">
                      <div className="flex items-center justify-end w-full gap-4">
                        <Link
                          href={`/dashboard/admin/banner/update/${item?._id}`}
                        >
                          <LiaEditSolid className="text-info-base text-2xl" />
                        </Link>
                        <button onClick={() => handleDeleteBanners(item)}>
