...
const UpdateTrustUs = ({ id }) => {
  ...
  const { data, isLoading: fetchLoading, error } = useGetSingleTrustUsQuery(id);

  const [updateTrustUs, { isLoading }] = useUpdateTrustUsMutation();
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (res?.success === true) {
        router.back();
        toast.success("Trust Us updated successfully!", {
  ...
  return (
    <div className="max-w-[1000px] bg-black-muted text-[#ADB5BD] mx-auto my-10 p-8 rounded-lg">
      <h1 className="text-[#ADB5BD] text-[23px] font-bold">Update Trust Us</h1>
      <form
        onSubmit={handleSubmit(onSubmit)}
        className="flex flex-col space-y-3 mt-4"
      >
        <div className="flex flex-col gap-2">
          {/* Input for Trust Us  title */}
          <div className="relative w-full">
            <span className="text-[16px] py-2 block">Trust Us title *</span>
            <input
              ...
              type="text"
              placeholder="Please enter your Trust Us  title"
              {...register("title", {
                required: "Trust Us  title is required",
              })}
            />
            {errors.title && (
