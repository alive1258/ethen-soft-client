...
const Admins = () => {
  ...
  const { data, error, isLoading } = useGetAllCustomerQuery({ page });
  console.log(data);
  ...
  const handleDelete = async (user) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the name with"${user?.email}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteCustomer(user?._id).unwrap();
        if (response?.success === true) {
          Swal.fire({
            title: "Deleted!",
            text: ` "${user?.fullName}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
  ...
  return (
    <>
      <div className="h-[75px] md:px-10 mt-5 rounded-t-[26px]  px-3 text-[#ADB5BD] bg-black-muted">
