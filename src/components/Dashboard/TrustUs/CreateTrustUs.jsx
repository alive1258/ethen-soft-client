...
import { useCreateTrustUsMutation } from "@/redux/api/trustUsApi";
...
const CreateTrustUs = () => {
  ...
  const [createTrustUs, { isLoading }] = useCreateTrustUsMutation();

  const router = useRouter();

  const onSubmit = async (data) => {
    try {
      const res = await createTrustUs({ ...data }).unwrap();
      if (res?.success === true) {
        reset();
        router.back();
        toast.success("trust us added successfully!", {
