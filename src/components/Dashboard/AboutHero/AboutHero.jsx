...
const AboutHero = () => {
  const { data, error, isLoading } = useGetAllAboutHeroQuery();
  const [deleteAboutHero] = useDeleteAboutHeroMutation();

  if (error) {
    console.error("Error fetching about hero descriptions:", error);
    return <p>Error fetching About Hero descriptions.</p>;
  }

  const handleDeleteAboutHero = async (data) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the About Hero "${data?.title}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteAboutHero(data?._id).unwrap();
        if (response?.success === true) {
          Swal.fire({
            title: "Deleted!",
            text: `The About Hero "${data?.title}" has been successfully deleted.`,
            icon: "success",
          });
        } else {
          Swal.fire({
      ...
      console.error("Delete About Hero error:", error);
      Swal.fire({
        title: "Error!",
    ...
    <>
      <div className="h-[75px] md:px-10 mt-5 rounded-t-[26px] flex justify-between items-center px-3 text-[#ADB5BD] bg-black-muted">
        <div className="flex items-center gap-1 text-[#ADB5BD] text-base font-semibold">
          <PiAlarmFill />
          <span>All About Hero</span>
        </div>
        <Link
          href={"/dashboard/admin/about/about-heros/create"}
          className="flex items-center gap-1 cursor-pointer text-[#4D69FA] bg-[#F0EFFB] rounded-[50px] py-1 px-[10px]"
        >
          <MdOutlineCloudUpload />
          <span className="text-[13px] font-semibold">Add About Hero</span>
        </Link>
      </div>
            ...
            <table className="min-w-full bg-black-muted text-[#ADB5BD] border-gray-200 mb-10 rounded-lg">
              <thead>
                <tr className="bg-black-muted text-start text-[13px] overflow-hidden">
                  ...
                  <th className="py-4 px-4 text-start">
                    <span>service_one</span>
                  </th>
                  <th className="py-4 px-4 text-start">
                    <span>service_two</span>
                  </th>
              ...
              <tbody>
                {data?.data?.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-4">{item?.service_one}</td>
                    <td className="py-3 px-4">{item?.service_two}</td>
                    <td className="my-2 px-4 text-end rounded-r-xl">
                      <div className="flex items-center justify-end w-full gap-4">
                        <Link
                          href={`/dashboard/admin/about/about-heros/update/${item?._id}`}
                        >
                          <LiaEditSolid className="text-info-base text-2xl" />
                        </Link>
                        <button onClick={() => handleDeleteAboutHero(item)}>
