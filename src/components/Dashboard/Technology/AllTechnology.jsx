...
const AllTechnology = () => {
  const { data, error, isLoading } = useGetAllTechnologyQuery();

  const [deleteTechnology] = useDeleteTechnologyMutation();

  const handleDeleteTechnology = async (data) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the  Technology "${data?.name}"?`,
        icon: "warning",
  ...
  return (
    <>
      <div className="h-[75px] md:px-10 mt-5 rounded-t-[26px] flex justify-between items-center px-3 text-[#ADB5BD] bg-black-muted">
        ...
        <Link
          href={"/dashboard/admin/home/technology/create"}
          className="flex items-center gap-1 cursor-pointer text-[#4D69FA] bg-[#F0EFFB] rounded-[50px] py-1 px-[10px]"
        >
          <MdOutlineCloudUpload />
          <span className="text-[13px] font-semibold">Add Technology</span>
        </Link>
      </div>
                  ...
                  >
                    ...
                    <td className="my-2 px-4 text-end rounded-r-xl">
                      <div className="flex items-center justify-end w-full gap-4">
                        <Link
                          href={`/dashboard/admin/home/technology/update/${item?._id}`}
                        >
                          <LiaEditSolid className="text-info-base text-2xl" />
                        </Link>
                        <button onClick={() => handleDeleteTechnology(item)}>
