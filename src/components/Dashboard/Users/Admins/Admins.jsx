  ...
  const handleDelete = async (user) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Are you sure you want to delete the name with"${user?.fullName}"?`,
        icon: "warning",
      ...
      if (result.isConfirmed) {
        const response = await deleteAdmin(user?._id).unwrap();
